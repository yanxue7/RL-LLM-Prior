...
def explore_rollout_q(env, embed_tkn, embed_policy, Q,
    ...
    while not done:
        ...
        if steps>50:
            break
    if (total_reward >0.5) or bad_example_t <= good_example_t or ('bala' not in alg):
            if total_reward >0.5:
                good_example_t += 1
            else:
                bad_example_t += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument("--embedding_llm_path", type=str, default='/xxx/agents/bert-base-uncased')
    ...
    if args.load_checkpoint is not None:
        prob_network = torch.load(f'/mnt/nasdata/yanxue/llm-prior/{args.load_checkpoint}').to(device)
    ...
    for iter in range(epoch):
        ...
        if len(replay_buffer) <= batch_size or iter<20 or iter%5!=0:#20 change to 40 for reuse
            continue
